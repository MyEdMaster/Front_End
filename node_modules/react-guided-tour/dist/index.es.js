...
var hopscotch_min = createCommonjsModule(function (module, exports) {
/**! hopscotch - v0.3.1
*
...
var HopscotchContext = function (_PureComponent) {
  inherits(HopscotchContext, _PureComponent);

  function HopscotchContext(props, context) {
    classCallCheck(this, HopscotchContext);

    var _this = possibleConstructorReturn(this, (HopscotchContext.__proto__ || Object.getPrototypeOf(HopscotchContext)).call(this, props, context));
    ...
    if (_this.props.bubbleWidth) tour['bubbleWidth'] = _this.props.bubbleWidth;
    if (_this.props.bubblePadding) tour['bubblePadding'] = _this.props.bubblePadding;
    if (_this.props.smoothScroll) tour['smoothScroll'] = _this.props.smoothScroll;
    if (_this.props.scrollDuration) tour['scrollDuration'] = _this.props.scrollDuration;
    if (_this.props.scrollTopMargin) tour['scrollTopMargin'] = _this.props.scrollTopMargin;
    if (_this.props.showCloseButton) tour['showCloseButton'] = _this.props.showCloseButton;
    if (_this.props.showPrevButton) tour['showPrevButton'] = _this.props.showPrevButton;
    if (_this.props.showNextButton) tour['showNextButton'] = _this.props.showNextButton;
    if (_this.props.arrowWidth) tour['arrowWidth'] = _this.props.arrowWidth;
    if (_this.props.skipIfNoElement) tour['skipIfNoElement'] = _this.props.skipIfNoElement;
    if (_this.props.nextOnTargetClick) tour['nextOnTargetClick'] = _this.props.nextOnTargetClick;
    if (_this.props.onNext) tour['onNext'] = _this.props.onNext;
    if (_this.props.onPrev) tour['onPrev'] = _this.props.onPrev;
    if (_this.props.onStart) tour['onStart'] = _this.props.onStart;
    if (_this.props.onEnd) tour['onEnd'] = _this.props.onEnd;
    if (_this.props.onClose) tour['onClose'] = _this.props.onClose;
    if (_this.props.onError) tour['onError'] = _this.props.onError;

    if (_this.props.i18n) {
      var i18n = {};
      if (_this.props.nextBtn) i18n['nextBtn'] = _this.props.nextBtn;
      if (_this.props.prevBtn) i18n['prevBtn'] = _this.props.prevBtn;
      if (_this.props.doneBtn) i18n['doneBtn'] = _this.props.doneBtn;
      if (_this.props.skipBtn) i18n['skipBtn'] = _this.props.skipBtn;
      if (_this.props.closeTooltip) i18n['closeTooltip'] = _this.props.closeTooltip;
      if (_this.props.stepNums) i18n['stepNums'] = _this.props.stepNums;
      tour['i18n'] = i18n;
    }
  ...
  createClass(HopscotchContext, [{
    key: 'getChildContext',
    value: function getChildContext() {
      return {
        tour: this.state.tour,
        customRenderer: this.state.customRenderer,
        hopscotch: hopscotch_min
      };
    }
  }, {
    ...
    value: function startTour() {
      var state = hopscotch_min.getState();
      if (this.props.active && (!state || state.indexOf(this.props.id + ':') === 0)) {
        if (this.state.customRenderer) {
          hopscotch_min.setRenderer(this.state.customRenderer);
        }
        hopscotch_min.startTour(this.state.tour);
      }
    }
  }, {
    key: 'render',
...
var nextOnCallback = function nextOnCallback(tour, callback) {
  var currentStep = hopscotch_min.getCurrStepNum();

  callback(function () {
    hopscotch_min.startTour(tour, currentStep);
  });
};

var StartButton = function (_Component) {
  inherits(StartButton, _Component);

  function StartButton(props, context) {
    classCallCheck(this, StartButton);

    var _this = possibleConstructorReturn(this, (StartButton.__proto__ || Object.getPrototypeOf(StartButton)).call(this, props, context));

    _this.startTour = function () {
      if (_this.context.customRenderer) {
        _this.context.hopscotch.setRenderer(_this.context.customRenderer);
      }
      _this.context.hopscotch.startTour(_this.context.tour);
    };

    return _this;
  }

  createClass(StartButton, [{
    key: 'render',
    value: function render() {
      ...
      return React.createElement(
        'button',
        { className: className, onClick: this.startTour },
        children
      );
    }
  }]);
...
export { HopscotchContext, StartButton, escapeHtml, waitForElementVisible, nextOnCallback };
